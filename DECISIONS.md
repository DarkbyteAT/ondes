...
**Scannability: enabled but not implemented (verified, with caveat).** The uniform layer pytree structure inside one body means `jax.lax.scan` over layers 1..N-1 works cleanly — the array leaves stack along a leading axis and the scan body is one `eqx.combine` step. `tests/test_scannability.py` demonstrates this for `SIREN`/`HSIREN`/`WIRE` (with and without FiLM) and asserts numerical match with the eager for-loop to within float32 fusion-reorder noise. The caveat: layer 0 carries `is_first=True` as a static field, distinct from `is_first=False` on the rest, so a naive `tree_map(stack, *body.layers)` over all N layers fails on the static-field mismatch. Realistic scan pattern: apply layer 0 separately, scan the remaining N-1. `ondes` does not ship a scan implementation — `Body.trunk` uses a plain Python loop — because (a) downstream renderers (loom) can choose between scan, unrolled loop, or `jax.lax.fori_loop` per their compile-budget needs, and (b) for typical 4-8 layer INRs the unrolled forward is what XLA produces anyway. The scannability is a *capability*, not a default. Forward-pass microbenchmark for the canonical `SIREN(in_dim=2, hidden_dim=64, num_hidden_layers=4)` × `batch=1024` config sits at ~0.5 ms median (jit+vmap, CPU, JAX 0.10) — no measurable difference from a hypothetical pre-refactor `BasisBody(kind="siren")` version, since `kind` was already static and XLA traced the conditional away.
