...
def test_mfn_body_conforms_to_basis_module_and_body(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=3, key=jax.random.PRNGKey(0))
...
def test_mfn_body_forward_scalar_shape(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(1))
...
def test_mfn_body_forward_vector_shape(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(2), out_features=4)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (4,)
...
def test_mfn_body_trunk_shape(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=32, num_hidden_layers=3, key=jax.random.PRNGKey(3))
...
def test_mfn_body_film_modulation_changes_output(body_cls):
    ...
    body = body_cls(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.PRNGKey(4))
...
def test_mfn_body_jit_matches_eager(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(5))
...
def test_mfn_body_grad_is_finite_and_nonzero(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(6))
...
def test_mfn_body_vmap_over_coords(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(7))
...
def test_mfn_body_rejects_zero_hidden_layers(body_cls):
    ...
    with pytest.raises(AssertionError):
        body_cls(in_dim=2, hidden_dim=8, num_hidden_layers=0, key=jax.random.PRNGKey(0))
...
def test_fourier_mfn_has_n_plus_one_filters():
    ...
    body = FourierMFN(in_dim=2, hidden_dim=16, num_hidden_layers=4, key=jax.random.PRNGKey(0))
...
def test_gabor_mfn_has_n_plus_one_filters_of_gabor_type():
    ...
    body = GaborMFN(in_dim=2, hidden_dim=16, num_hidden_layers=3, key=jax.random.PRNGKey(0))
...
def test_fourier_filter_bias_uniform_phase():
    ...
    f = FourierFilter(in_dim=2, hidden_dim=2048, input_scale=256.0, n_layers=4, key=jax.random.PRNGKey(0))
...
def test_fourier_filter_weight_scale_matches_paper_formula():
    ...
    f = FourierFilter(in_dim=4, hidden_dim=256, input_scale=input_scale, n_layers=n_layers, key=jax.random.PRNGKey(1))
...
def test_gabor_filter_mu_within_unit_box():
    ...
    g = GaborFilter(in_dim=3, hidden_dim=512, n_layers=4, key=jax.random.PRNGKey(0))
...
def test_gabor_filter_gamma_is_strictly_positive():
    ...
    g = GaborFilter(in_dim=2, hidden_dim=1024, n_layers=4, key=jax.random.PRNGKey(1))
...
def test_fourier_mfn_no_unused_omega_field_on_filter():
    ...
    f = FourierFilter(in_dim=2, hidden_dim=4, input_scale=4.0, n_layers=1, key=jax.random.PRNGKey(0))
...
def test_recurrence_state_stacked_along_axis_0():
    ...
    body = FourierMFN(in_dim=2, hidden_dim=8, num_hidden_layers=4, key=jax.random.PRNGKey(0))
