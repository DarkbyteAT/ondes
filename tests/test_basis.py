...
def test_siren_init_first_layer_uses_one_over_in_dim_bound():
    ...
    W, b = siren_init(in_dim, out_dim, omega, is_first=True, key=jax.random.key(0))
...
def test_siren_init_hidden_layer_uses_sqrt6_over_omega_bound():
    ...
    W, b = siren_init(in_dim, out_dim, omega, is_first=False, key=jax.random.key(1))
...
def test_basis_layer_each_kind_produces_finite_output(layer_cls):
    ...
    layer = layer_cls(in_dim, out_dim, omega_init=6.0, is_first=True, key=jax.random.key(7))
    y = layer(x)
...
def test_basis_layer_is_subclass_of_basis_abc(layer_cls):
    ...
    layer = layer_cls(2, 4, omega_init=6.0, is_first=True, key=jax.random.key(0))
...
def test_basis_subclass_dispatch_correct_activation():
    ...
    siren = SIRENLayer(1, 1, omega_init=1.0, is_first=True, key=jax.random.key(0))
    hsiren = HSIRENLayer(1, 1, omega_init=1.0, is_first=True, key=jax.random.key(0))
    wire = WIRELayer(1, 1, omega_init=1.0, is_first=True, key=jax.random.key(0), s_init=1.0)
...
def test_wire_layer_s_init_is_threaded():
    ...
    layer = WIRELayer(2, 4, 6.0, is_first=True, key=jax.random.key(0), s_init=2.5)
...
def test_wire_body_s_init_threads_to_layers():
    ...
    body = WIRE(in_dim=2, hidden_dim=8, num_hidden_layers=3, key=jax.random.key(0), s_init=2.5)
...
def test_basis_module_protocol_matches_concrete_bodies():
    ...
    args = dict(in_dim=2, hidden_dim=8, num_hidden_layers=2, key=jax.random.key(0))
...
def test_basis_body_call_is_jit_compilable():
    ...
    body = SIREN(in_dim=2, hidden_dim=32, num_hidden_layers=3, key=jax.random.key(2))
...
def test_non_wire_body_parameter_count_excludes_s(body_cls):
    ...
    in_dim, hidden_dim, num_layers = 3, 16, 4
    body = body_cls(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.key(3))
    expected = 0
    for i in range(num_layers):
        in_d = in_dim if i == 0 else hidden_dim
        expected += hidden_dim * in_d  # W
...
def test_basis_body_film_modulation_changes_output():
    ...
    body = SIREN(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.key(4))
...
def test_basis_body_out_features_none_returns_scalar():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(5))
...
def test_basis_body_out_features_int_returns_vector():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(6), out_features=4)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (4,)
...
def test_basis_body_trunk_returns_hidden_features():
    ...
    hidden_dim = 32
    coord = jnp.array([0.25, -0.5])
    scalar_body = SIREN(in_dim=2, hidden_dim=hidden_dim, num_hidden_layers=3, key=jax.random.key(7))
    vector_body = SIREN(in_dim=2, hidden_dim=hidden_dim, num_hidden_layers=3, key=jax.random.key(7), out_features=5)
...
def test_basis_body_trunk_is_jit_compilable(body_cls):
    ...
    body = body_cls(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(8))
...
def test_basis_body_trunk_with_film_modulation_changes_output():
    ...
    in_dim, hidden_dim, num_layers = 2, 8, 2
    body = SIREN(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.key(9))
    coord = jnp.array([0.3, -0.7])
    film = jnp.ones((num_layers, 2 * hidden_dim)) * 0.5
...
def test_basis_body_out_features_one_returns_scalar():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(10), out_features=1)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == ()
...
def test_basis_body_out_features_two_returns_vector():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(11), out_features=2)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (2,)
...
def test_basis_body_grad_through_scalar_path():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(12))
    coord = jnp.array([0.3, -0.4])
...
def test_basis_body_grad_through_vector_path():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(13), out_features=4)
    coord = jnp.array([0.3, -0.4])
...
def test_basis_body_vmap_over_coords_scalar():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(14))
    coords = jax.random.uniform(jax.random.key(140), (7, 2), minval=-1.0, maxval=1.0)
...
def test_basis_body_vmap_over_coords_vector():
    ...
    body = SIREN(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(15), out_features=3)
    coords = jax.random.uniform(jax.random.key(150), (7, 2), minval=-1.0, maxval=1.0)
    out = jax.vmap(body)(coords)
...
def test_basis_body_scalar_path_squeeze_only_feature_dim():
    ...
    body = SIREN(in_dim=2, hidden_dim=8, num_hidden_layers=2, key=jax.random.key(16))
