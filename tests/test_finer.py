...
def test_finer_layer_is_basis_subclass() -> None:
    ...
    layer = FINERLayer(2, 8, omega_init=30.0, is_first=True, key=jax.random.key(0))
...
def test_finer_activation_matches_paper_formula() -> None:
    ...
    layer = FINERLayer(1, 1, omega_init=1.0, is_first=True, key=jax.random.key(0), scale_req_grad=True)
...
def test_finer_first_layer_bias_within_first_bias_scale() -> None:
    ...
    fbs = 10.0
    layer = FINERLayer(3, 1024, omega_init=30.0, is_first=True, key=jax.random.key(0), first_bias_scale=fbs)
...
def test_finer_hidden_layer_bias_uses_siren_bound_not_first_bias_scale() -> None:
    ...
    fbs = 20.0
    in_dim = 64
    layer = FINERLayer(in_dim, 256, omega_init=omega, is_first=False, key=jax.random.key(0), first_bias_scale=fbs)
...
def test_finer_body_is_basis_module() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(0))
...
def test_finer_body_forward_scalar_shape() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(1))
...
def test_finer_body_forward_vector_shape() -> None:
    ...
    # When: forward-passing
    # Then: output shape (3,)
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(2), out_features=3)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (3,)
...
def test_finer_body_trunk_shape() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=32, num_hidden_layers=3, key=jax.random.key(3))
...
def test_finer_body_first_bias_scale_threads_to_first_layer_only() -> None:
    ...
    fbs = 8.0
    body = FINER(in_dim=2, hidden_dim=64, num_hidden_layers=3, key=jax.random.key(0), first_bias_scale=fbs)
...
def test_finer_body_film_modulation_changes_output() -> None:
    ...
    body = FINER(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.key(4))
...
def test_finer_body_jit_matches_eager() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(5))
...
def test_finer_body_grad_is_finite_and_nonzero() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(6))
...
def test_finer_body_vmap_over_coords() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.key(7))
...
def test_finer_body_layer_pytree_homogeneous() -> None:
    ...
    body = FINER(in_dim=2, hidden_dim=64, num_hidden_layers=4, key=jax.random.key(0))
...
def test_finer_body_rejects_zero_hidden_layers() -> None:
    ...
    with pytest.raises(AssertionError):
        FINER(in_dim=2, hidden_dim=8, num_hidden_layers=0, key=jax.random.key(0))
