...
def test_bacon_body_conforms_to_basis_module_and_body() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=3, key=jax.random.PRNGKey(0))
...
def test_bacon_body_forward_scalar_shape() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(1))
...
def test_bacon_body_forward_vector_shape() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(2), out_features=3)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (3,)
...
def test_bacon_filter_frequencies_are_integer_multiples_of_quantization() -> None:
    ...
    dq = 2.0 * float(jnp.pi)
    f = BACONFilter(in_dim=2, hidden_dim=256, bandwidth=10 * dq, quantization_interval=dq, key=jax.random.PRNGKey(0))
...
def test_bacon_filter_frequencies_within_per_layer_bandwidth() -> None:
    ...
    dq = 2.0 * float(jnp.pi)
    bandwidth = 5 * dq
    f = BACONFilter(in_dim=3, hidden_dim=512, bandwidth=bandwidth, quantization_interval=dq, key=jax.random.PRNGKey(1))
...
def test_bacon_filter_frequencies_include_both_signs_and_zero() -> None:
    ...
    dq = 1.0
    f = BACONFilter(in_dim=2, hidden_dim=4096, bandwidth=4 * dq, quantization_interval=dq, key=jax.random.PRNGKey(2))
...
def test_bacon_output_bandwidth_property_equals_sum_of_per_layer() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=8, num_hidden_layers=4, key=jax.random.PRNGKey(0), max_freq=128.0)
...
def test_bacon_has_n_plus_one_filters() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=4, key=jax.random.PRNGKey(0))
...
def test_bacon_body_film_modulation_changes_output() -> None:
    ...
    body = BACON(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.PRNGKey(4))
...
def test_bacon_body_jit_matches_eager() -> None:
    ...
    # When: jit-compiling
    # Then: matches eager
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(5))
...
def test_bacon_body_grad_is_finite_and_nonzero() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(6))
...
def test_bacon_body_vmap_over_coords() -> None:
    ...
    body = BACON(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(8))
    coords = jax.random.uniform(jax.random.PRNGKey(80), (4, 2), minval=-1.0, maxval=1.0)
...
def test_bacon_body_rejects_zero_hidden_layers() -> None:
    ...
    with pytest.raises(AssertionError):
        BACON(in_dim=2, hidden_dim=8, num_hidden_layers=0, key=jax.random.PRNGKey(0))
