...
def test_pnf_body_conforms_to_basis_module_and_body():
    ...
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=3, key=jax.random.PRNGKey(0))
...
def test_pnf_body_forward_scalar_shape():
    ...
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(1))
...
def test_pnf_body_forward_vector_shape():
    ...
    # When: forward-passing
    # Then: output shape (4,)
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(2), out_features=4)
    y = body(jnp.array([0.1, -0.2]))
    assert y.shape == (4,)
...
def test_pnf_body_trunk_shape():
    ...
    body = PNF(in_dim=2, hidden_dim=32, num_hidden_layers=3, key=jax.random.PRNGKey(3))
...
def test_pnf_has_mix_matrix_per_recurrence_step():
    ...
    body = PNF(in_dim=2, hidden_dim=8, num_hidden_layers=4, key=jax.random.PRNGKey(0))
...
def test_pnf_has_n_plus_one_fourier_filters():
    ...
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=4, key=jax.random.PRNGKey(0))
...
def test_pnf_mix_matrix_changes_output_vs_zero_mix():
    ...
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=3, key=jax.random.PRNGKey(7))
...
def test_pnf_body_film_modulation_changes_output():
    ...
    body = PNF(in_dim=in_dim, hidden_dim=hidden_dim, num_hidden_layers=num_layers, key=jax.random.PRNGKey(4))
...
def test_pnf_body_jit_matches_eager():
    ...
    # When: jit-compiling
    # Then: matches eager
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(5))
...
def test_pnf_body_grad_is_finite_and_nonzero():
    ...
    # parameter sub-tree).
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(6))
    coord = jnp.array([0.3, -0.4])
...
def test_pnf_mix_matrix_receives_gradient():
    ...
    body = PNF(in_dim=2, hidden_dim=8, num_hidden_layers=2, key=jax.random.PRNGKey(11))
    coord = jnp.array([0.3, -0.4])
...
def test_pnf_body_vmap_over_coords():
    ...
    body = PNF(in_dim=2, hidden_dim=16, num_hidden_layers=2, key=jax.random.PRNGKey(7))
...
def test_pnf_body_rejects_zero_hidden_layers():
    ...
    with pytest.raises(AssertionError):
        PNF(in_dim=2, hidden_dim=8, num_hidden_layers=0, key=jax.random.PRNGKey(0))
