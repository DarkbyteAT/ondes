...
class WinnerSchedule(eqx.Module):
    ...
    ``2 · (channel-averaged-bin-mean) / n_channels²``. The reference
...
def spectral_centroid(
    ...
    *,
    ...
    Computes ``(Σ f · |X(f)| / Σ |X(f)|) · 2 / n_channels``, averaged over
    every non-channel-non-frequency axis and over channels. The factor of
    2 and the ``/ n_channels`` match the reference implementation
    (``modules/networks.py`` ``spectral_centroid``):

    - per channel, rFFT along ``freq_axis``, take magnitudes
    - weight bins by ``rfftfreq(n_freq, d=1)`` and reduce over all axes
      except the channel axis (numerator/denominator separately, with a
      zero-safe divide)
    - average across channels
    - multiply by 2, divide by ``n_channels``
    ...
    Args:
        signal: Real-valued signal of any rank. The frequency axis (the
            one rFFT is taken along) and the channel axis (the last axis
            in the reference, but configurable) must both be present.
    ...
    Note:
        ...
        layout. For 1-D mono audio with shape ``(n_samples,)``,
        broadcast/reshape to ``(n_samples, 1)`` first (channel axis
        present at the end) and call with default axes.
    """
    if signal.ndim < 2:
        raise ValueError(
    ...
    # rfftfreq with d=1 matches the reference convention
    ...
    # Reduce over every axis except channel — that matches the reference
    # behaviour where the per-channel centroid is the average over rows
    # (and within rows over freq bins), with channels averaged at the end.
    ...
    return cast(Float[Array, ""], (centroid * 2.0) / n_channels)
...
def _perturb_first_two_layers(
    ...
    noise0 = jax.random.normal(k0, layers[0].W.shape, dtype=layers[0].W.dtype) * scale0.astype(layers[0].W.dtype)
    noise1 = jax.random.normal(k1, layers[1].W.shape, dtype=layers[1].W.dtype) * scale1.astype(layers[1].W.dtype)
...
class WINNER(Body):
    ...
    def __init__(
        ...
        *,
        ...
        assert num_hidden_layers >= 2, (
            f"WINNER perturbs layers 0 and 1; need num_hidden_layers >= 2, got {num_hidden_layers}"
        )
        out_features_canon = _validate_body_args(num_hidden_layers, out_features)

        s0_arr = jnp.asarray(s0, dtype=jnp.float32) if not isinstance(s0, jax.Array) else s0
        s1_arr = jnp.asarray(s1, dtype=jnp.float32) if not isinstance(s1, jax.Array) else s1
        ...
        per_layer_keys = jax.random.split(k_layers, num_hidden_layers)
        ...
        layers = _perturb_first_two_layers(layers, s0_arr, s1_arr, omega_hidden, k_noise)

        self.layers = layers
        self.readout_W, self.readout_b = _build_readout(hidden_dim, omega_hidden, out_features_canon, k_readout)
        self.out_features = out_features_canon
    ...
    def reset_noise(self, key: Key[Array, ""]) -> "WINNER":
        ...
        is split into ``num_hidden_layers`` per-layer subkeys. Only the
        first two layer subkeys and ``k_noise`` are used here. Consequence:
        ``WINNER(key=k, ...).reset_noise(k)`` is bit-identical to the
        original WINNER for layers 0 and 1 — the determinism property the
        regression test relies on.
        """
        k_layers, _k_readout, k_noise = jax.random.split(key, 3)
        per_layer_keys = jax.random.split(k_layers, self.num_hidden_layers)
        ...
        layers = _perturb_first_two_layers(layers, self.s0, self.s1, self.omega_hidden, k_noise)
