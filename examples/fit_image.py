...
def make_coords(grid_n: int, in_dim: int):
    """Build a regular grid in `[-1, 1]^in_dim` and return `(N, in_dim)` coords.
    ...
    consistent.
    """
    axes = [jnp.linspace(-1.0, 1.0, grid_n)] * in_dim
...
def synthetic_target(name: str, grid_n: int):
    ...
    coords = make_coords(grid_n, in_dim=2)
...
def load_image(path: Path, grid_n: int):
    ...
    `(x, y) → amplitude`.
    """
    img = np.asarray(Image.open(path).resize((grid_n, grid_n)), dtype=np.float32) / 255.0
    if img.ndim == 2:
        coords = make_coords(grid_n, in_dim=2)
        return coords, jnp.asarray(img.ravel())
    # RGB: (H, W, 3); flatten to (H*W*3, 3) coords + (H*W*3,) amplitudes.
    h, w, c = img.shape
    xs = jnp.linspace(-1.0, 1.0, h)
    ys = jnp.linspace(-1.0, 1.0, w)
    cs = jnp.linspace(-1.0, 1.0, c)
    gx, gy, gc = jnp.meshgrid(xs, ys, cs, indexing="ij")
    coords = jnp.stack([gx.ravel(), gy.ravel(), gc.ravel()], axis=-1)
    return coords, jnp.asarray(img.ravel())
...
def train(model, coords, target, *, steps: int, lr: float):
    ...
    initial_loss = float(loss_fn(model, coords, target))
    for _ in range(steps):
        model, opt_state, _ = step(model, opt_state, coords, target)
    final_loss = float(loss_fn(model, coords, target))
    return model, initial_loss, final_loss


def reconstruct(model, grid_n: int, in_dim: int):
    """Evaluate the trained model on a regular grid; returns `(grid_n, ...)` array."""
    coords = make_coords(grid_n, in_dim)
    flat = jax.vmap(model)(coords)
    shape = (grid_n,) * in_dim
...
def main(
    ...
    if output is not None:
        ...
        if in_dim == 3:
            recon = recon.reshape(grid, grid, grid)  # (H, W, C)
